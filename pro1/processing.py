...
if __name__=="__main__":
    targetFolder="D:\github\Data\shumo\data\hour"
    ...
    for key in Station_ID.keys():
        ...
        # First_Year=1900
        # Last_Year=2019
        First_Year = Station_ID[key]['HLY First Year']
        Last_Year = Station_ID[key]['HLY Last Year']
        ...
        for year in range(int(First_Year),int(Last_Year+1)):
            url = "http://climate.weather.gc.ca/climate_data/bulk_data_e.html?format=csv&stationID={}&Year={}&Month=1&Day=14&timeframe=1&submit=Download+Data"\
                .format(key,year)
            print(url)
            wget.download(url, out=os.path.join(targetFolder,str(key),"{}.csv".format(year)))
